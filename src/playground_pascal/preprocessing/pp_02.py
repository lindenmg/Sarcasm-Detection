...
# Number of maximum examples. -1 if you don't want to use all preprocessing
max_train = -1
max_test = -1
...
def get_tfidf(train=True, test=True, data_size=None, max_tfidf_features=int(1e4)):
    ...
    vocab = create_vocab(string_list, pp, min_df=1, ngram_range=(2, 4), analyzer='char',
                         max_tfidf_features=None, load_cache=False, create_cache=False)
    ...
    if train:
        ...
        train_post_tfidf = create_tfidf(train_post_strings, 'post', pp, vocab=vocab, min_df=10, ngram_range=(2, 4),
                                        analyzer='char', max_tfidf_features=max_tfidf_features)
        train_reply_tfidf = create_tfidf(train_reply_strings, 'reply', pp, vocab=vocab, min_df=10, ngram_range=(2, 4),
                                         analyzer='char', max_tfidf_features=max_tfidf_features)
    ...
    if test:
        if not (test_post_strings and test_reply_strings):
            ...
            test_post_strings = pp.inner_str_join_2d_list(test_post_lemma, concat_text_str=' ')
            test_reply_strings = pp.inner_str_join_2d_list(test_reply_lemma, concat_text_str=' ')
        test_post_tfidf = create_tfidf(test_post_strings, 'post', pp, vocab=vocab, min_df=10, ngram_range=(2, 4),
                                       analyzer='char', max_tfidf_features=max_tfidf_features)
        test_reply_tfidf = create_tfidf(test_reply_strings, 'reply', pp, vocab=vocab, min_df=10, ngram_range=(2, 4),
                                        analyzer='char', max_tfidf_features=max_tfidf_features)
